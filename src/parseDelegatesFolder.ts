...
export function parseDelegateFolder(
  delegatesFolderPath: string,
  folder: string
): Delegate {
  ...
  const cumemberFilePath = path.join(
    delegatesFolderPath,
    folder,
    "cumember.md"
  );
  ...
  const imageFilePath = image
    ? path.join(delegatesFolderPath, folder, image)
    : "";
  ...
  return {
    ...
    image: imageFilePath,
    metrics,
    cuMember: fs.existsSync(cumemberFilePath),
  };
}
