...
const verbose_handler_1 = require("./verbose-handler");
const options = {
    flags: {
        output: {
            type: 'string',
...
async function handleInput({ flags }) {
    if (flags.pack) {
        const { root, filename } = await (0, fs_1.packToFs)({ input: flags.pack, output: flags.output, wrapWithDirectory: flags.wrapWithDirectory, customStreamSink: flags.verbose ? verbose_handler_1.printUnixFsContent : undefined });
        ...
        return (0, lib_1.listFilesAndCidsInCar)({ input: flags.listFull });
    }
    else if (flags.hash) {
        return (0, lib_1.hashCar)({ input: flags.hash });
    }
